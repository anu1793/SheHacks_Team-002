...
Original file is located at
    https://colab.research.google.com/drive/14rJvNisapdSZWual81h6l4NzvZZUxgcr
"""
...
for i in range(10):
    k = x_val.sample(1).index[0]
    print("Book: ", books['book_name'][k], 
          "\nPredicted genre: ", predict1(x_val[k])) ,
    print("Actual genre: ",books['genre_new'][k], "\n")
...
LogReg_pipeline.fit(x_train, ytrain)
...
def predict(m):
    ...
    m = clean_summary(m)
    m = remove_stopwords(m)
    m_pred=clf.predict(m)
    return multilabel_binarizer.inverse_transform(m_pred)

for i in range(10):
    k = x_val.sample(1).index[0]
    print(predict(x_val[k]))
    print("Book: ", books['book_name'][k], 
          "\nPredicted genre: ", predict(x_val[k])) ,
    print("Actual genre: ",books['genre_new'][k], "\n")
